...
// Vercel Pro: allow up to 120s for complex multi-tool AI agent reports
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Use text() first to safely handle empty or invalid JSON bodies
    const rawText = await response.text();
    console.log('n8n raw response:', rawText.substring(0, 200));
    ...
    if (rawText && rawText.trim()) {
      try {
        ...
        console.error('n8n JSON parse error:', parseErr, 'raw:', rawText.substring(0, 200));
      }
    }
